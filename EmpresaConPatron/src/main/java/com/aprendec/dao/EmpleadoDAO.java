...
public class EmpleadoDAO {
	 ...
	 * @return true si la operación de actualización fue exitosa; false de lo
	 *         contrario.
	 ...
	 * 
	 * @return una lista de objetos que contiene todos los empleados. Si no se
	 *         encuentran empleados, devuelve una lista vacía.
	 * 
	 * @throws SQLException si ocurre un error durante la consulta a la base de
	 *                      datos.
	 */
	public List<Empleado> obtenerEmpleados(String orden, String direccion)
		...
		try {
			...
			while (resultSet.next()) {
				...
				em.setSexo(resultSet.getString(3).charAt(0)); // Se obtiene un carácter
			...
			e.printStackTrace();
			throw new SQLException("Error obteniendo empleados: " + e.getMessage());
		} finally {
			if (resultSet != null)
				resultSet.close();
			if (statement != null)
	 ...
	 * @return un objeto que representa el empleado correspondiente al dni
	 *         proporcionado. Si no se encuentra un empleado con ese DNI, devuelve
	 *         un empleado vacío.
	 * 
	 * @throws SQLException si ocurre un error durante la consulta a la base de
	 *                      datos.
	...
	/**
	 * Busca empleados en la base de datos según los parámetros de búsqueda.
	 * 
	 ...
	 * 
	 * @return una lista de empleados que cumplen con los criterios especificados.
	 * @throws SQLException si ocurre un error de SQL durante la consulta.
	 */
	public List<Empleado> buscarEmpleados(String sexo, String nombre, String categoria, String anyos)
			throws SQLException {
		...
		try {
			connection = obtenerConexion(); // Obtén la conexión desde el singleton
